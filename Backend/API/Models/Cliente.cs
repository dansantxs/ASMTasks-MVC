...
namespace API.Models
{
    public class Cliente
    {
        ...
        public string? RG { get; set; }                         // Opcional (somente PF)
        public string? InscricaoEstadual { get; set; }          // Opcional (somente PJ)
        ...
        private void ValidarDados()
        {
            ...
            if (string.IsNullOrWhiteSpace(TipoPessoa) || (TipoPessoa != "F" && TipoPessoa != "J"))
            ...
            if (TipoPessoa == "F")
            {
                // Documento deve ser CPF válido
                if (!ValidarCPF(docNumerico))
                    throw new ValidationException("CPF inválido.");

                // Se inscrição estadual vier preenchida para PF, não é permitido
                ...
                // RG é opcional, mas você poderia validar formato/tamanho se quiser:
                // if (!string.IsNullOrWhiteSpace(RG) && RG.Length < 3) ...
            }

            if (TipoPessoa == "J")
            {
                // Documento deve ser CNPJ válido
                if (!ValidarCNPJ(docNumerico))
                    throw new ValidationException("CNPJ inválido.");

                // Se RG vier preenchido para PJ, não é permitido
                if (!string.IsNullOrWhiteSpace(RG))
                    throw new ValidationException("RG não deve ser informado para pessoa jurídica.");

                // Inscrição Estadual é opcional, mas se vier vazia está tudo bem.
                // Se quiser validar algo (ex: tamanho mínimo), pode fazer aqui:
                // if (!string.IsNullOrWhiteSpace(InscricaoEstadual) && InscricaoEstadual.Length < 3) ...
            }
        }
