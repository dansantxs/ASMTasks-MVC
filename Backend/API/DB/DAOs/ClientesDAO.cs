...
namespace API.DB.DAOs
{
    public class ClientesDAO
    {
        public async Task<int> CriarAsync(DBContext dbContext, Cliente cliente)
        {
            ...
            cmd.CommandText = @"
                INSERT INTO Cliente
                (Nome, Documento, TipoPessoa, RG, InscricaoEstadual, Email, Telefone, CEP, Cidade, UF, Logradouro, Bairro, Numero, Ativo)
                VALUES
                (@Nome, @Documento, @TipoPessoa, @RG, @InscricaoEstadual, @Email, @Telefone, @CEP, @Cidade, @UF, @Logradouro, @Bairro, @Numero, @Ativo);
        ...
        public async Task<IEnumerable<Cliente>> ObterTodosAsync(DBContext dbContext)
        {
            ...
            while (await dr.ReadAsync())
            {
                lista.Add(new Cliente
                {
                    ...
                    TipoPessoa = dr["TipoPessoa"].ToString() ?? string.Empty,
                    RG = dr["RG"]?.ToString(),
