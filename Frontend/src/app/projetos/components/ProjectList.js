...
export default function ProjectList({
  ...
  }, [searchTerm, projects, clientesById, setoresById]);

  const projetosAtivos = filteredProjects.filter((item) => item.ativo && !projetoEstaConcluido(item, etapasById));
  const projetosConcluidos = filteredProjects.filter((item) => item.ativo && projetoEstaConcluido(item, etapasById));
  const projetosInativos = filteredProjects.filter((item) => !item.ativo);
