...
export default function FormularioPrioridade({
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      ...
      const nameExists = prioridadesExistentes.some(
        p =>
          p.name.toLowerCase() === formData.name.toLowerCase() &&
          p.id !== (prioridade && prioridade.id)
      );
      if (nameExists) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>
            {prioridade ? 'Editar Prioridade' : 'Cadastrar Nova Prioridade'}
          </DialogTitle>
          <DialogDescription>
            {prioridade
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <Button type="submit" className="bg-brand-blue hover:bg-brand-blue-dark">
              {prioridade ? 'Salvar Alterações' : 'Cadastrar Prioridade'}
            </Button>
          </div>
        </form>
      </DialogContent>
